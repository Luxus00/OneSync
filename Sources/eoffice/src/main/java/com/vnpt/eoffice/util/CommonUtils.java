...
public class CommonUtils {
    ...
    public static String getLikeAfterCondition(String str) {
        if (str == null) {
            str = "";
        }
        if (!str.trim().isEmpty()) {
    ...
    public static char stripUTF8Char(char ch) {
        ...
        char result = ch;
        int index = Arrays.binarySearch(SOURCE_CHARACTERS, result);
        if (index >= 0) {
...
//    }
    ...
    public static void filter(Object value, PreparedStatement preparedStatement, int index) throws Exception {
        if (value != null) {
            if (value instanceof Date) {
                ...
                preparedStatement.setObject(index, new Timestamp(temp.getTime()));
            } else {
    ...
    public static String getZoneName(HttpServletRequest request) {
        try {
            return request.getHeader("zoneName");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return null;
        }
    ...
    public static String LatOrLngToString(Double value) {
        if (value == null) {
            return "0.00000";
        }
        DecimalFormat format = new DecimalFormat("######.#####");
    ...
    public static String getLikeConditionAfter(String str) {
        if (str == null) {
            str = "";
        }
        if (!str.trim().isEmpty()) {
    ...
    public static boolean safeEqual(Long obj1, Long obj2) {
        ...
        return ((null != obj1) && (null != obj2) && obj1.compareTo(obj2) == 0);
    }
    ...
    /**
     * Check for number in the range fromNumber through toNumber
     *
     * @param fromNumber
     * @param toNumber
     * @param number
     ...
     */
    public static boolean isTheRange(int fromNumber, int toNumber, int number) {
        return number >= fromNumber && number <= toNumber ? true : false;
    }

    public static long transferHourMinuteToMinute(int hour, int minute) {
